...
execute as @s at @s if score @s crossbowReloads matches 1.. run data merge entity @e[type=armor_stand,tag=CrossbowReloader,limit=1] {equipment:{mainhand:{components:{charged_projectiles:[{id:"minecraft:arrow",components:{intangible_projectile:{}}}]}}}}
execute as @s at @s if score @s crossbowReloads matches ..0 run data merge entity @e[type=armor_stand,tag=CrossbowReloader,limit=1] {equipment:{mainhand:{components:{"minecraft:custom_data":{is_active:true}}}}}
