...
execute as @e[type=armor_stand,tag=TheTurret] at @s if score @s turret_id = @a[tag=TheTurretor,limit=1] id store result score @e[tag=DespawnTurretor,limit=1] turret_health run scoreboard players get @s turret_health
