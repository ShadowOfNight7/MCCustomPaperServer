execute as @s at @s on vehicle run data merge entity @s {Item:{components:{attribute_modifiers:[{type:max_health,slot:armor,amount:1,operation:add_value,id:"1"}]}}}
...
execute as @s at @s on vehicle run data merge entity @s {Item:{components:{"minecraft:custom_data":{slowness:1b,passive_ability:true,plate_applied:1}}}}