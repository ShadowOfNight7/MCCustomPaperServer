...
execute if score @s enchant.counter matches 4..12 as @a at @s if score @s id = @e[type=#gameify:mobs,tag=CounterMarker,limit=1] enchant.counter2 positioned as @e[type=#gameify:mobs,tag=CounterMarker,limit=1] run title @s actionbar [{"text":">>>    Prepare...    <<<","color":"#cac644"}]
